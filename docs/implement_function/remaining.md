...
| 項目 | 状態 |
|---|---|
| A-1(persist) / A-5コア / A-6(整理) / A-8(一部) / A-9(vitest計21) / B-1(backup) / B-2(redo) / B-3(保存状態) / **B-4(PNG書出)** / **B-5(sync整合性チェッカー)** | ✅ 実装済 |
...
**この環境で検証できる高価値（🟢）:**
1. ~~B-4（PNG書き出し）~~ ✅ / ~~B-5（sync整合性チェッカー）~~ ✅
2. **B-6（遭遇検出タイムライン）** — 同室/すれ違いを自動抽出（本作の推理の核・自己完結）
